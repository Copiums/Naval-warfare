...
EventManager:AddEvent(runService.RenderStepped, function()
        ...
        for _, obj in next, workspace:GetChildren() do
                ...
                if not table.find(Ship.ShipNames, obj.Name) then 
                        continue; 
                end;
                if not obj:FindFirstChild("Team") then 
                        continue; 
                end;
                if obj.Team.Value == lplr.Team.Name then 
                        continue;
                end;
        ...
        if targetType == "plane" then
                ...
                local leadDist: number = (barrel.Position - leadPos).Magnitude;
                local leadAngle: number = Calculations.Artillery:angle(leadDist, 800);
                local leadFlightTime: number = Calculations.Artillery:flightTime(leadAngle, 800);
                leadPos = closestTarget.Position + (targetVelocity * leadFlightTime);
                aimPos = leadPos;
        end;
...
EventManager:AddEvent(runService.RenderStepped, function()
        ...
        for _, plane in next, workspace:GetChildren() do
                ...
                if not string.match(plane.Name, "Bomber") then 
                        continue; 
                end;
...
EventManager:AddEvent(lplr.Chatted, function(msg: string)
        ...
        if msg == ";b" then
                ...
                if island and island:FindFirstChild("MainBody") then 
                        Shoot(island.MainBody.Position) 
                end;
        elseif msg == ";a" then
